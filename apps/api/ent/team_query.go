...
type TeamQuery struct {
	...
	withInvites     *TeamInviteQuery
	withRoles       *TeamRoleQuery
...
func (_q *TeamQuery) QueryInvites() *TeamInviteQuery {
	query := (&TeamInviteClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TeamQuery) QueryRoles() *TeamRoleQuery {
	query := (&TeamRoleClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TeamQuery) WithInvites(opts ...func(*TeamInviteQuery)) *TeamQuery {
	query := (&TeamInviteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) WithRoles(opts ...func(*TeamRoleQuery)) *TeamQuery {
	query := (&TeamRoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	...
	if query := _q.withInvites; query != nil {
		if err := _q.loadInvites(ctx, query, nodes,
			func(n *Team) { n.Edges.Invites = []*TeamInvite{} },
			func(n *Team, e *TeamInvite) { n.Edges.Invites = append(n.Edges.Invites, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withRoles; query != nil {
		if err := _q.loadRoles(ctx, query, nodes,
			func(n *Team) { n.Edges.Roles = []*TeamRole{} },
			func(n *Team, e *TeamRole) { n.Edges.Roles = append(n.Edges.Roles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TeamQuery) loadInvites(ctx context.Context, query *TeamInviteQuery, nodes []*Team, init func(*Team), assign func(*Team, *TeamInvite)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(teaminvite.FieldTeamID)
	}
	query.Where(predicate.TeamInvite(func(s *sql.Selector) {
...
func (_q *TeamQuery) loadRoles(ctx context.Context, query *TeamRoleQuery, nodes []*Team, init func(*Team), assign func(*Team, *TeamRole)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(teamrole.FieldTeamID)
	}
	query.Where(predicate.TeamRole(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.RolesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
