...
func (_c *RoleCreate) AddUserBindingIDs(ids ...string) *RoleCreate {
	_c.mutation.AddUserBindingIDs(ids...)
	return _c
}

// AddUserBindings adds the "user_bindings" edges to the UserRoleBinding entity.
func (_c *RoleCreate) AddUserBindings(v ...*UserRoleBinding) *RoleCreate {
	ids := make([]string, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddUserBindingIDs(ids...)
}
...
func (_c *RoleCreate) createSpec() (*Role, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.UserBindingsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
