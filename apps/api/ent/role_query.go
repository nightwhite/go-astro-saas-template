...
type RoleQuery struct {
	...
	withUserBindings *UserRoleBindingQuery
...
// QueryUserBindings chains the current query on the "user_bindings" edge.
func (_q *RoleQuery) QueryUserBindings() *UserRoleBindingQuery {
...
func (_q *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withUserBindings: _q.withUserBindings.Clone(),
...
// WithUserBindings tells the query-builder to eager-load the nodes that are connected to
// the "user_bindings" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithUserBindings(opts ...func(*UserRoleBindingQuery)) *RoleQuery {
	query := (&UserRoleBindingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withUserBindings = query
	return _q
}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := _q.withUserBindings; query != nil {
		if err := _q.loadUserBindings(ctx, query, nodes,
			func(n *Role) { n.Edges.UserBindings = []*UserRoleBinding{} },
			func(n *Role, e *UserRoleBinding) { n.Edges.UserBindings = append(n.Edges.UserBindings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RoleQuery) loadUserBindings(ctx context.Context, query *UserRoleBindingQuery, nodes []*Role, init func(*Role), assign func(*Role, *UserRoleBinding)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userrolebinding.FieldRoleID)
	}
	query.Where(predicate.UserRoleBinding(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.UserBindingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
